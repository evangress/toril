...
impl<'a> VariantStrIter<'a> {
    ...
    fn impl_get(&self, i: usize) -> &'a str {
        unsafe {
            // SECURITY (GHSA-wrw7-89jp-8q8g): `g_variant_get_child` writes the
            // string pointer back through this out-argument. It must be passed
            // as `&mut p`; the upstream `&p` was unsound (the write was UB and,
            // under optimization, dropped — leaving `p` NULL and crashing
            // `CStr::from_ptr` below). Fixed in glib 0.20.0 but never backported
            // to the 0.18 line that gtk3-rs / Tauri require, so we vendor-patch.
