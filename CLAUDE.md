...
- **All GUI flows are unverified** (dialogs, Ctrl+S, tabs, sidebar, watcher reload, toolbar buttons + emoji picker + active-state highlighting) — they need the webview; see the build-environment note. Verify on a machine with platform webview deps. The toolbar's *command layer* is gated headlessly, but its DOM wiring (clicks, prompts, picker, focus return) is not.
...
pnpm test             # vitest — round-trip + toolbar + tabs + security (jsdom)
...
**Next: Phase 3 (continued)** — GFM (done) / emoji plugin (done; math **deferred**: its only plugin is deprecated — see §8) / formatting toolbar (done, gated). Remaining: Source / Typewriter / Focus modes, themes + persisted settings, `sanitize.ts` wired into all rendered content (§3.3), clipboard image paste, then HTML → PDF export.
...
**Phase 3 — MarkText parity** *(in progress: GFM, emoji, and the formatting toolbar are done + gated)*
